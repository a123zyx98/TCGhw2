...
int MCTS_move(int Board[BOUNDARYSIZE][BOUNDARYSIZE], clock_t end_t, int turn, int game_length,int GameRecord[MAXGAMELENGTH][BOUNDARYSIZE][BOUNDARYSIZE]){
    ...
    while(clock() < end_t){
        ...
        while(cur_node -> child.size()!=0){
            ...
            int index =0;
            for(int i=0;i<cur_node -> child.size();i++,index++){
                if(mu[i]*sigma[i] < PRUNE_SIGMA_E)continue;
                if(Mmr[index] < best_Mml){
                    cur_node -> child.erase(cur_node -> child.begin(), cur_node -> child.begin()+i);
                    i--;
                }
            }
